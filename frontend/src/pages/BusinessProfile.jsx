   ...
   async function getAuthToken() {
    ...
    useEffect(() => {
      ...
      async function fetchProfile() {
        ...
        if (!token) {
          console.warn(
            "No auth token available — cannot fetch BusinessProfile",
          );
          return;
        }
     ...
     function removeLocalFile(kind) {
      ...
      async function handleSave(e) {
        ...
        try {
          ...
          else if (meta.signatureUrl)
            fd.append("signatureUrl", meta.signatureUrl);
          ...
          const merged = {
            ...
            signatureOwnerName:
              saved.signatureOwnerName ?? meta.signatureOwnerName,
            signatureOwnerTitle:
              saved.signatureOwnerTitle ?? meta.signatureOwnerTitle,
            defaultTaxPercent:
              saved.defaultTaxPercent ?? meta.defaultTaxPercent,
            notes: saved.notes ?? meta.notes,
            profileId:
              saved._id ?? meta.profileId ?? saved.id ?? meta.profileId,
          };
          ...
          if (saved.logoUrl)
            setPreviews((p) => ({
              ...p,
              logo: resolveImageUrl(saved.logoUrl),
            }));
          if (saved.stampUrl)
            setPreviews((p) => ({
              ...p,
              stamp: resolveImageUrl(saved.stampUrl),
            }));
          ...
          alert(
            err?.message || "Failed to save profile. See console for details.",
          );
        } finally {
          setSaving(false);
        }
    ...
    <div className={businessProfileStyles.pageContainer}>
      <div className={businessProfileStyles.headerContainer}>
        ...
        <p className={businessProfileStyles.headerSubtitle}>
          Manage your business information, logo, and other settings here.
        </p>

        {!SignedIn && (
          <div className="mt-3 bg-blue-800 p-2">
            <p>Please sign in to view and edit your business profile.</p>
          </div>
        )}
      </div>
      ...
      >
        ...
        <div className={businessProfileStyles.cardContainer}>
          ...
          <div classNName={businessProfileStyles.gridCols2Lg}>
                ...
                <div className={businessProfileStyles.taxContainer}>
                  ...
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      onChange={(e) => {
                        updateMeta("defaultTaxPercent", Number(e.target.value));
                      }}
                    />
                    <span className={customStyles.taxPercentage}>%</span>
                  </div>
                  <p classNAme={businessProfileStyles.taxHelpText}>
                    This tax percentage will be applied by default to line items
                    in new invoices. You can override it per item when creating
                    or editing an invoice.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className={businessProfileStyles.actionContainer}>
          <div className={businessProfileStyles.actionInnerContainer}>
            <div className={businessProfileStyles.actionButtonGroup}>
              <button type='submit' disabled={saving} className={businessProfileStyles.saveButton} onClick={handleSave}>
