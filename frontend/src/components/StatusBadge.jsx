...
const StatusBadge = ({status = "" , size = "default" , showIcon = true}) => {
  ...
  return (
    <div
      className={`inline-flex items-center ${sizeClasses[size]} rounded-full font-medium ${config.bg} ${config.text} border ${config.bprder} transition-all duration-300 ease-in-out `}
    >
      <div className={`absolute inset-0 bg-linear-to-r ${config.gradient} opacity-0 group-hover:opacity-5 `}>
      </div>
      ...
      <span className='relative z-10 font-semibold capitalize tracking-wide'>
        {s === 'default' ? status : s}
      </span>
